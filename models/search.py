...
class Search():
    ...
    def __init__(self, query):
        self.query = query
        self.url_encoded_query = urllib.parse.urlencode({'q': 'site:divinityoriginalsin2.wiki.fextralife.com {}'.format(query)})
        self.results = self.performSearch()

    def performSearch(self):
        page_data = requests.get('https://www.google.com/search?{}'.format(self.url_encoded_query))
        ...
        GOOGLE_RE = re.compile(r'<span class="st">\S\S\S \S?\S, \S\S\S\S \*\*...\*\*|<\/span>')
        ...
        for result in soup.findAll('div', class_="g", id_=None)[:5]:
            if result.a != "imagebox_bigimages":
                ...
                search_embed.add_field(
                    name="{}".format(title), value="[{}]({})\n{}".format(link_tree.a['href'][7:-83], link_tree.a['href'][7:-83], cleanhtml(description)), inline=False
                )
        return search_embed
